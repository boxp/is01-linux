...
grep -F 'DT_MACHINE_START' "$patch_note" >/dev/null || fail 'patch note does not describe DT machine descriptor'
grep -F 'sharp,is01' "$patch_note" >/dev/null || fail 'patch note does not describe IS01 DT compatible'
grep -F 'setup_machine_fdt()' "$patch_note" >/dev/null || fail 'patch note does not describe setup_machine_fdt checkpoint'
grep -F 'non-NULL mdesc' "$patch_note" >/dev/null || fail 'patch note does not describe non-NULL mdesc checkpoint'
grep -F 'before fdt_totalsize()/memblock_reserve()' "$patch_note" >/dev/null || fail 'patch note does not describe pre-memblock reserve checkpoint'
...
printf 'Phase 3 deckard-dt-mdesc reset candidate verified:\n'
