...
kernel_out=${KERNEL_OUT:-build/phase3/atag-core-magic-reset-kernel-out}
...
out_dir='build/phase3/atag-core-magic-reset'
...
patch_note="$out_dir/atag-core-magic-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-atag-core-magic-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_atag_core_magic_reset:" in text:
    raise SystemExit("error: IS01 atag-core-magic reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_atag_core_magic_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 atag-core-magic proc_comm reset probe needs non-LPAE section mappings
#endif
...
\t * dt_virt pointer whose first tag header matches ATAG_CORE.
\t * Trigger the downstream proc_comm reset before early_init_dt_verify()
\t * so the next split can confirm whether the FDT path is receiving
\t * an ATAG list.
\t */
\tasm volatile(
\t\"mrs r0, cpsr\\n\"
...
\tif (((const struct tag_header *)dt_virt)->tag == ATAG_CORE) {
%s
\t}
...
note.write_text(
    ...
    "to trigger proc_comm reset when setup_machine_fdt() receives a non-NULL "
    "dt_virt pointer whose first ATAG header tag matches ATAG_CORE, before "
    "early_init_dt_verify() tries to validate the pointer as an FDT.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-atag-core-magic-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-atag-core-magic-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-atag-core-magic-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-atag-core-magic-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-atag-core-magic-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-atag-core-magic-reset-ubinize.cfg"
...
printf 'Phase 3 atag-core-magic reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
