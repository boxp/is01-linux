...
kernel_out=${KERNEL_OUT:-build/phase3/decompressor-pre-kernel-reset-kernel-out}
...
out_dir='build/phase3/decompressor-pre-kernel-reset'
...
patch_note="$out_dir/decompressor-pre-kernel-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-decompressor-pre-kernel-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM decompressor pre-decompress_kernel marker")
if ".Lis01_decompressor_pre_kernel_reset:" in text:
    raise SystemExit("error: IS01 decompressor pre-kernel reset patch is already present")

reset = """
\t\t/*
\t\t * IS01 Phase 3 probe: if the real Linux decompressor reaches the
\t\t * pre-decompress_kernel handoff point after relocation, BSS clear,
\t\t * and cache setup, trigger the downstream proc_comm reset path.
\t\t */
.Lis01_decompressor_pre_kernel_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\t.ltorg
"""
insertion = reset + "\n" + "\t\tb\t.Lis01_decompressor_pre_kernel_reset\n"
head.write_text(text.replace(needle, insertion + needle, 1))
note.write_text(
    "Patched arch/arm/boot/compressed/head.S before decompress_kernel() "
    "to trigger proc_comm reset after decompressor relocation/BSS/cache setup.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-decompressor-pre-kernel-reset-ubinize.cfg"
...
printf 'Phase 3 decompressor pre-kernel reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
