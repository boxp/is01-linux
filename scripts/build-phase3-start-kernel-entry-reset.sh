...
kernel_out=${KERNEL_OUT:-build/phase3/start-kernel-entry-reset-kernel-out}
...
out_dir='build/phase3/start-kernel-entry-reset'
...
patch_note="$out_dir/start-kernel-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-start-kernel-entry-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_start_kernel_reset:" in text:
    raise SystemExit("error: IS01 start-kernel-entry reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_start_kernel_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 start-kernel proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: if execution reaches start_kernel(),
\t * trigger the downstream proc_comm reset path. The early ARM
\t * page tables above keep the physical proc_comm regions reachable.
\t */
\tasm volatile(
\t\t\"mrs r0, cpsr\\n\"
\t\t\"orr r0, r0, #0xc0\\n\"
...
main_needle = """void start_kernel(void)
{
"""
if main_needle not in main_text:
    raise SystemExit("error: could not find start_kernel function entry")
if "IS01 Phase 3 probe: if execution reaches start_kernel()" in main_text:
    raise SystemExit("error: IS01 start-kernel-entry reset patch is already present")
main_c.write_text(
    main_text.replace(
        main_needle,
...
note.write_text(
    ...
    "trigger proc_comm reset at start_kernel entry.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-start-kernel-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-start-kernel-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-start-kernel-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-start-kernel-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-start-kernel-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-start-kernel-entry-reset-ubinize.cfg"
...
printf 'Phase 3 start-kernel-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
