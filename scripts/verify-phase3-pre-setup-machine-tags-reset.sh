...
grep -F 'returns NULL' "$patch_note" >/dev/null || fail 'patch note does not describe NULL mdesc checkpoint'
grep -F 'before the setup_machine_tags() fallback' "$patch_note" >/dev/null || fail 'patch note does not describe pre-setup_machine_tags checkpoint'
...
printf 'Phase 3 pre-setup-machine-tags reset candidate verified:\n'
