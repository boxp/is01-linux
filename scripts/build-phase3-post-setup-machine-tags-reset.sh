...
kernel_out=${KERNEL_OUT:-build/phase3/post-setup-machine-tags-reset-kernel-out}
...
out_dir='build/phase3/post-setup-machine-tags-reset'
...
patch_note="$out_dir/post-setup-machine-tags-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-setup-machine-tags-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_post_setup_machine_tags_reset:" in text:
    raise SystemExit("error: IS01 post-setup-machine-tags reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_post_setup_machine_tags_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 post-setup-machine-tags proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: setup_machine_tags() returned to setup_arch().
\t * Trigger downstream proc_comm reset immediately after that fallback
\t * returns, before invalid-machine handling.
...
if "IS01 Phase 3 probe: setup_machine_tags() returned to setup_arch()" in setup_text:
    raise SystemExit("error: IS01 post-setup-machine-tags reset patch is already present")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately after setup_machine_tags() "
    "returns, before invalid-machine handling.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-setup-machine-tags-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-setup-machine-tags-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-setup-machine-tags-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-setup-machine-tags-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-setup-machine-tags-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-setup-machine-tags-reset-ubinize.cfg"
...
printf 'Phase 3 post-setup-machine-tags reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
