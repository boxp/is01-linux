...
kernel_out=${KERNEL_OUT:-build/phase3/post-mmu-control-write-reset-kernel-out}
...
out_dir='build/phase3/post-mmu-control-write-reset'
...
patch_note="$out_dir/post-mmu-control-write-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-mmu-control-write-reset-stock-kernel-slot.bin"
...
if control_needle not in text:
    raise SystemExit("error: could not find ARM post-mmu-control-write marker")
if map_needle not in text:
    raise SystemExit("error: could not find ARM __create_page_tables return marker")
if ".Lis01_post_mmu_control_write_reset:" in text or ".Lis01_map_proc_comm_for_post_mmu_reset:" in text:
    raise SystemExit("error: IS01 post-mmu-control-write reset patch is already present")
...
reset = """
		/*
		 * IS01 Phase 3 probe: if __turn_mmu_on returns from the MMU
		 * control register write with the temporary proc_comm identity
		 * mappings active, trigger the downstream proc_comm reset path.
		 */
.Lis01_post_mmu_control_write_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
note.write_text(
    ...
    "writing the MMU-enable control register.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-mmu-control-write-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-mmu-control-write-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-mmu-control-write-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-mmu-control-write-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-mmu-control-write-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-mmu-control-write-reset-ubinize.cfg"
...
printf 'Phase 3 post-mmu-control-write reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
