...
kernel_out=${KERNEL_OUT:-build/phase3/machine-desc-resolved-reset-kernel-out}
...
out_dir='build/phase3/machine-desc-resolved-reset'
...
patch_note="$out_dir/machine-desc-resolved-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-machine-desc-resolved-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_machine_desc_resolved_reset:" in text:
    raise SystemExit("error: IS01 machine-desc-resolved reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_machine_desc_resolved_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 machine-desc-resolved proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: if execution reaches the point where
\t * setup_arch() has resolved a usable machine descriptor, trigger the
\t * downstream proc_comm reset path before dereferencing mdesc.
...
if "IS01 Phase 3 probe: if execution reaches the point where" in setup_text:
    raise SystemExit("error: IS01 machine-desc-resolved reset patch is already present")
...
if count != 1:
    raise SystemExit("error: could not find machine_desc assignment in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately before machine_desc = mdesc.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-machine-desc-resolved-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-machine-desc-resolved-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-machine-desc-resolved-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-machine-desc-resolved-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-machine-desc-resolved-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-machine-desc-resolved-reset-ubinize.cfg"
...
printf 'Phase 3 machine-desc-resolved reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
