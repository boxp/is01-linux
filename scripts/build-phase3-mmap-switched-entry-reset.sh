...
kernel_out=${KERNEL_OUT:-build/phase3/mmap-switched-entry-reset-kernel-out}
...
out_dir='build/phase3/mmap-switched-entry-reset'
head_s="$kernel_src/arch/arm/kernel/head.S"
head_common_s="$kernel_src/arch/arm/kernel/head-common.S"
...
test -s "$head_common_s" || {
  printf 'error: missing Linux ARM kernel common head: %s\n' "$head_common_s" >&2
  exit 1
}

mkdir -p "$kernel_out" "$out_dir"
patch_note="$out_dir/mmap-switched-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-mmap-switched-entry-reset-stock-kernel-slot.bin"
...
restore_head() {
  ...
  if [ -n "${head_common_backup:-}" ] && [ -f "$head_common_backup" ]; then
    cp "$head_common_backup" "$head_common_s"
    rm -f "$head_common_backup"
  fi
}
...
head_common_backup=$(mktemp)
cp "$head_common_s" "$head_common_backup"

python3 - "$head_s" "$head_common_s" "$patch_note" <<'PY'
from pathlib import Path
import sys

head = Path(sys.argv[1])
head_common = Path(sys.argv[2])
...
if ".Lis01_map_proc_comm_for_mmap_switched_reset:" in text:
    raise SystemExit("error: IS01 mmap-switched-entry reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_mmap_switched_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 mmap-switched proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
		/*
		 * IS01 Phase 3 probe: if execution reaches __mmap_switched
		 * after returning from the idmap MMU-enable path, trigger the
		 * downstream proc_comm reset path.
		 */
.Lis01_mmap_switched_entry_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
common_text = head_common.read_text()
common_needle = """__mmap_switched:

\tmov\tr7, r1
"""
if common_needle not in common_text:
    raise SystemExit("error: could not find ARM __mmap_switched entry marker")
if ".Lis01_mmap_switched_entry_reset:" in common_text:
    raise SystemExit("error: IS01 mmap-switched-entry reset patch is already present")
head_common.write_text(
    common_text.replace(
        common_needle,
        "__mmap_switched:\n" + reset + "\n\tmov\tr7, r1\n",
        1,
...
note.write_text(
    ...
    "and MSM_CSR in early page tables, then patched "
    "arch/arm/kernel/head-common.S to trigger proc_comm reset at "
    "__mmap_switched entry.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-mmap-switched-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-mmap-switched-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-mmap-switched-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-mmap-switched-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-mmap-switched-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-mmap-switched-entry-reset-ubinize.cfg"
...
printf 'Phase 3 mmap-switched-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
