...
kernel_out=${KERNEL_OUT:-build/phase3/fdt-pointer-nonnull-reset-kernel-out}
...
out_dir='build/phase3/fdt-pointer-nonnull-reset'
...
patch_note="$out_dir/fdt-pointer-nonnull-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-fdt-pointer-nonnull-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_fdt_pointer_nonnull_reset:" in text:
    raise SystemExit("error: IS01 fdt-pointer-nonnull reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_fdt_pointer_nonnull_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 fdt-pointer-nonnull proc_comm reset probe needs non-LPAE section mappings
#endif
...
\t * dt_virt pointer. Trigger the downstream proc_comm reset before
\t * early_init_dt_verify() so the next split can distinguish missing
\t * pointer from invalid-DTB failure.
\t */
\tasm volatile(
\t\"mrs r0, cpsr\\n\"
\t\"orr r0, r0, #0xc0\\n\"
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately after setup_machine_fdt() confirms "
    "dt_virt is non-NULL, before early_init_dt_verify() validates whether the "
    "pointer is a valid FDT.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-fdt-pointer-nonnull-reset-ubinize.cfg"
...
printf 'Phase 3 fdt-pointer-nonnull reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
