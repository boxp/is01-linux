...
kernel_out=${KERNEL_OUT:-build/phase3/decompressor-post-kernel-reset-kernel-out}
...
out_dir='build/phase3/decompressor-post-kernel-reset'
...
patch_note="$out_dir/decompressor-post-kernel-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-decompressor-post-kernel-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """\t\tbl\tdecompress_kernel

\t\tget_inflated_image_size\tr1, r2, r3
"""
if needle not in text:
    raise SystemExit("error: could not find ARM decompressor post-decompress_kernel marker")
if ".Lis01_decompressor_post_kernel_reset:" in text:
    raise SystemExit("error: IS01 decompressor post-kernel reset patch is already present")

reset = """
\t\t/*
\t\t * IS01 Phase 3 probe: if the real Linux decompressor reaches the
\t\t * post-decompress_kernel point before cache clean/off and final
\t\t * kernel entry, trigger the downstream proc_comm reset path.
\t\t */
.Lis01_decompressor_post_kernel_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\t.ltorg
"""
replacement = "\t\tbl\tdecompress_kernel\n" + reset + "\n" + "\t\tget_inflated_image_size\tr1, r2, r3\n"
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/boot/compressed/head.S immediately after decompress_kernel() "
    "returns to trigger proc_comm reset before cache clean/off and __enter_kernel.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-decompressor-post-kernel-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-decompressor-post-kernel-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-decompressor-post-kernel-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-decompressor-post-kernel-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-decompressor-post-kernel-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-decompressor-post-kernel-reset-ubinize.cfg"
...
printf 'Phase 3 decompressor post-kernel reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
