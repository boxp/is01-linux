...
grep -F 'valid FDT' "$patch_note" >/dev/null || fail 'patch note does not describe valid FDT checkpoint'
grep -F 'before of_flat_dt_match_machine()' "$patch_note" >/dev/null || fail 'patch note does not describe pre-machine-match checkpoint'
...
printf 'Phase 3 fdt-verify-ok reset candidate verified:\n'
