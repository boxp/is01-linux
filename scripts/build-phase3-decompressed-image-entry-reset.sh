...
kernel_out=${KERNEL_OUT:-build/phase3/decompressed-image-entry-reset-kernel-out}
...
out_dir='build/phase3/decompressed-image-entry-reset'
...
patch_note="$out_dir/decompressed-image-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-decompressed-image-entry-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM decompressed kernel stext marker")
if ".Lis01_decompressed_image_entry_reset:" in text:
    raise SystemExit("error: IS01 decompressed-image-entry reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if execution reaches the decompressed
		 * kernel image at stext, trigger the downstream proc_comm reset
		 * path before normal kernel entry setup.
		 */
.Lis01_decompressed_image_entry_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
note.write_text(
    "Patched arch/arm/kernel/head.S at ENTRY(stext) "
    "to trigger proc_comm reset at decompressed kernel image entry.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-decompressed-image-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-decompressed-image-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-decompressed-image-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-decompressed-image-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-decompressed-image-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-decompressed-image-entry-reset-ubinize.cfg"
...
printf 'Phase 3 decompressed-image-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
