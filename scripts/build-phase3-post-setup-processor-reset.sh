...
if ".Lis01_map_proc_comm_for_post_setup_processor_reset:" in text:
    raise SystemExit("error: IS01 post-setup-processor reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_post_setup_processor_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 post-setup-processor proc_comm reset probe needs non-LPAE section mappings
#endif
...
\t * after setup_processor(), trigger the downstream proc_comm reset path.
...
if "IS01 Phase 3 probe: if execution reaches the point immediately" in setup_text:
    raise SystemExit("error: IS01 post-setup-processor reset patch is already present")
...
if count != 1:
    raise SystemExit("error: could not find setup_processor call in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately after setup_processor().\\n"
)
PY
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-setup-processor-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-setup-processor-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-setup-processor-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-setup-processor-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-setup-processor-reset-ubinize.cfg"
...
printf 'Phase 3 post-setup-processor reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
