...
kernel_out=${KERNEL_OUT:-build/phase3/decompressor-pre-enter-kernel-reset-kernel-out}
...
out_dir='build/phase3/decompressor-pre-enter-kernel-reset'
...
patch_note="$out_dir/decompressor-pre-enter-kernel-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-decompressor-pre-enter-kernel-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """\t\tbl\tcache_clean_flush
\t\tbl\tcache_off

#ifdef CONFIG_ARM_VIRT_EXT
"""
if needle not in text:
    raise SystemExit("error: could not find ARM decompressor pre-__enter_kernel marker")
if ".Lis01_decompressor_pre_enter_kernel_reset:" in text:
    raise SystemExit("error: IS01 decompressor pre-enter-kernel reset patch is already present")

reset = """
\t\t/*
\t\t * IS01 Phase 3 probe: if the real Linux decompressor reaches the
\t\t * pre-__enter_kernel point after cache clean/off, trigger the
\t\t * downstream proc_comm reset path before final kernel entry.
\t\t */
.Lis01_decompressor_pre_enter_kernel_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\t.ltorg
"""
replacement = "\t\tbl\tcache_clean_flush\n\t\tbl\tcache_off\n" + reset + "\n" + "#ifdef CONFIG_ARM_VIRT_EXT\n"
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/boot/compressed/head.S after cache_clean_flush/cache_off "
    "to trigger proc_comm reset before __enter_kernel.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-decompressor-pre-enter-kernel-reset-ubinize.cfg"
...
printf 'Phase 3 decompressor pre-enter-kernel reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
