...
grep -F 'setup_machine_fdt() returned NULL' "$patch_note" >/dev/null || fail 'patch note does not describe setup_machine_fdt fallback checkpoint'
grep -F 'setup_machine_tags()' "$patch_note" >/dev/null || fail 'patch note does not describe setup_machine_tags checkpoint'
grep -F 'MACH_DECKARD' "$patch_note" >/dev/null || fail 'patch note does not describe MACH_DECKARD checkpoint'
grep -F '__machine_arch_type' "$patch_note" >/dev/null || fail 'patch note does not describe __machine_arch_type checkpoint'
grep -F '2008030' "$patch_note" >/dev/null || fail 'patch note does not describe numeric DECKARD machine type'
...
printf 'Phase 3 machine-type-deckard reset candidate verified:\n'
