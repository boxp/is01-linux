...
rootfs_cpio="$rootfs_out/images/rootfs.cpio"
...
ubi_img="$out_dir/phase2-buildroot-stock-kernel-recovery.ubi"
...
test -s "$rootfs_cpio" || ./scripts/build-phase2-buildroot-rootfs.sh
...
file "$boot_img"
file "$recovery_img"
sha256sum "$stock_boot" "$rootfs_cpio" "$raw_ramdisk" "$boot_img" "$ubi_payload" "$recovery_img"
