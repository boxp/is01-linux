...
python3 - "$zimage_dtb" "$padded_kernel" "$stock_kernel_size" <<'PY'
from pathlib import Path
import sys
...
if len(data) > slot_size:
    raise SystemExit(f"kernel payload is larger than stock kernel slot: {len(data)} > {slot_size}")
...
build_candidate phase3-stockslot-lean-stock-cmdline ''
build_candidate phase3-stockslot-lean-phase3-cmdline "$phase3_cmdline"
...
printf 'Phase 3 stock-header Linux entry recovery candidates are ready:\n'
cat "$source_note"
