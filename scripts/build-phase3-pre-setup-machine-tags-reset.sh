...
kernel_out=${KERNEL_OUT:-build/phase3/pre-setup-machine-tags-reset-kernel-out}
...
out_dir='build/phase3/pre-setup-machine-tags-reset'
...
patch_note="$out_dir/pre-setup-machine-tags-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-pre-setup-machine-tags-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_pre_setup_machine_tags_reset:" in text:
    raise SystemExit("error: IS01 pre-setup-machine-tags reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_pre_setup_machine_tags_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 pre-setup-machine-tags proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: setup_machine_fdt() returned NULL, so
\t * setup_arch() is about to fall back to setup_machine_tags().
\t * Trigger the downstream proc_comm reset path before that call.
...
if "IS01 Phase 3 probe: setup_machine_fdt() returned NULL" in setup_text:
    raise SystemExit("error: IS01 pre-setup-machine-tags reset patch is already present")
...
if count != 1:
    raise SystemExit("error: could not find setup_machine_fdt pre-setup_machine_tags block in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset when setup_machine_fdt() returns NULL, "
    "immediately before the setup_machine_tags() fallback.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-pre-setup-machine-tags-reset-ubinize.cfg"
...
printf 'Phase 3 pre-setup-machine-tags reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
