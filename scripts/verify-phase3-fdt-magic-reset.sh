...
grep -F 'FDT_MAGIC' "$patch_note" >/dev/null || fail 'patch note does not describe FDT_MAGIC checkpoint'
grep -F 'early_init_dt_verify()' "$patch_note" >/dev/null || fail 'patch note does not describe early_init_dt_verify checkpoint'
grep -F 'FDT header' "$patch_note" >/dev/null || fail 'patch note does not describe FDT header validation'
...
printf 'Phase 3 fdt-magic reset candidate verified:\n'
