...
kernel_out=${KERNEL_OUT:-build/phase3/pre-enable-mmu-reset-kernel-out}
...
out_dir='build/phase3/pre-enable-mmu-reset'
...
patch_note="$out_dir/pre-enable-mmu-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-pre-enable-mmu-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """	ldr	r13, =__mmap_switched		@ address to jump to after
...
if needle not in text:
    raise SystemExit("error: could not find ARM pre-enable-mmu marker")
if ".Lis01_pre_enable_mmu_reset:" in text:
    raise SystemExit("error: IS01 pre-enable-mmu reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if decompressed kernel head.S returns
		 * from CPU-specific setup, trigger the downstream proc_comm
		 * reset path immediately before branching to __enable_mmu.
		 */
.Lis01_pre_enable_mmu_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\tb\t.Lis01_reset_loop
"""
replacement = needle.replace("1:\tb\t__enable_mmu", "1:" + reset + "\n\tb\t__enable_mmu")
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S immediately before the __enable_mmu "
    "branch to trigger proc_comm reset after CPU-specific setup returns.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-pre-enable-mmu-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-pre-enable-mmu-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-pre-enable-mmu-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-pre-enable-mmu-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-pre-enable-mmu-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-pre-enable-mmu-reset-ubinize.cfg"
...
printf 'Phase 3 pre-enable-mmu reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
