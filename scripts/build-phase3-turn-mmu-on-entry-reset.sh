...
kernel_out=${KERNEL_OUT:-build/phase3/turn-mmu-on-entry-reset-kernel-out}
...
out_dir='build/phase3/turn-mmu-on-entry-reset'
...
patch_note="$out_dir/turn-mmu-on-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-turn-mmu-on-entry-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """ENTRY(__turn_mmu_on)
	mov	r0, r0
"""
if needle not in text:
    raise SystemExit("error: could not find ARM turn-mmu-on-entry marker")
if ".Lis01_turn_mmu_on_entry_reset:" in text:
    raise SystemExit("error: IS01 turn-mmu-on-entry reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if execution enters __turn_mmu_on,
		 * trigger the downstream proc_comm reset path before writing
		 * the MMU-enable control register.
		 */
.Lis01_turn_mmu_on_entry_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\tb\t.Lis01_reset_loop
"""
replacement = needle.replace("ENTRY(__turn_mmu_on)", "ENTRY(__turn_mmu_on)" + reset)
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S at __turn_mmu_on entry to trigger "
    "proc_comm reset before writing the MMU-enable control register.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-turn-mmu-on-entry-reset-ubinize.cfg"
...
printf 'Phase 3 turn-mmu-on-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
