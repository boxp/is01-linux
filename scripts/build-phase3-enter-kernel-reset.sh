...
kernel_out=${KERNEL_OUT:-build/phase3/enter-kernel-reset-kernel-out}
...
out_dir='build/phase3/enter-kernel-reset'
...
patch_note="$out_dir/enter-kernel-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-enter-kernel-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM decompressor __enter_kernel marker")
if ".Lis01_decompressor_enter_kernel_reset:" in text:
    raise SystemExit("error: IS01 enter-kernel reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if the real Linux decompressor enters
		 * __enter_kernel, trigger the downstream proc_comm reset path
		 * before branching to the decompressed kernel image.
		 */
.Lis01_decompressor_enter_kernel_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
note.write_text(
    "Patched arch/arm/boot/compressed/head.S at __enter_kernel "
    "to trigger proc_comm reset before branching to the decompressed kernel.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-enter-kernel-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-enter-kernel-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-enter-kernel-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-enter-kernel-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-enter-kernel-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-enter-kernel-reset-ubinize.cfg"
...
printf 'Phase 3 enter-kernel reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
