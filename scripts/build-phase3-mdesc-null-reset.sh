...
kernel_out=${KERNEL_OUT:-build/phase3/mdesc-null-reset-kernel-out}
...
out_dir='build/phase3/mdesc-null-reset'
...
patch_note="$out_dir/mdesc-null-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-mdesc-null-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_mdesc_null_reset:" in text:
    raise SystemExit("error: IS01 mdesc-null reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_mdesc_null_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 mdesc-null proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: setup_machine_tags() returned NULL.
\t * Trigger downstream proc_comm reset immediately after entering
\t * invalid-machine handling.
...
if "IS01 Phase 3 probe: setup_machine_tags() returned NULL" in setup_text:
    raise SystemExit("error: IS01 mdesc-null reset patch is already present")
...
if count != 1:
    raise SystemExit("error: could not find invalid-machine handling in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately after entering invalid-machine "
    "handling when setup_machine_tags() returns NULL.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-mdesc-null-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-mdesc-null-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-mdesc-null-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-mdesc-null-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-mdesc-null-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-mdesc-null-reset-ubinize.cfg"
...
printf 'Phase 3 mdesc-null reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
