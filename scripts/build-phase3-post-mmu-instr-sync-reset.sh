...
kernel_out=${KERNEL_OUT:-build/phase3/post-mmu-instr-sync-reset-kernel-out}
...
out_dir='build/phase3/post-mmu-instr-sync-reset'
...
patch_note="$out_dir/post-mmu-instr-sync-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-mmu-instr-sync-reset-stock-kernel-slot.bin"
...
text = head.read_text()
control_needle = """	mcr	p15, 0, r0, c1, c0, 0		@ write control reg
...
"""
if control_needle not in text:
    raise SystemExit("error: could not find ARM post-mmu-instr-sync marker")
if map_needle not in text:
    raise SystemExit("error: could not find ARM __create_page_tables return marker")
if ".Lis01_post_mmu_instr_sync_reset:" in text or ".Lis01_map_proc_comm_for_post_mmu_reset:" in text:
    raise SystemExit("error: IS01 post-mmu-instr-sync reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_post_mmu_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 post-MMU proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
		/*
		 * IS01 Phase 3 probe: if __turn_mmu_on reaches the
		 * post-enable instruction synchronization before returning to
		 * the virtual continuation address, trigger the downstream
		 * proc_comm reset path.
		 */
.Lis01_post_mmu_instr_sync_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
replacement = control_needle.replace(
    "\tmov\tr3, r13",
    reset + "\n\tmov\tr3, r13",
)
head.write_text(text.replace(control_needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S to identity-map proc_comm shared RAM "
    "and MSM_CSR in early page tables, then trigger proc_comm reset after "
    "the post-MMU instruction synchronization and before returning to "
    "the virtual continuation address.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-mmu-instr-sync-reset-ubinize.cfg"
...
printf 'Phase 3 post-mmu-instr-sync reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
