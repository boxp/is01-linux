...
test -s "$zimage" || {
  printf 'error: missing lean kernel zImage: %s\n' "$zimage" >&2
  exit 1
}
test -s "$dtb" || {
  printf 'error: missing lean DTB: %s\n' "$dtb" >&2
  exit 1
}
...
build_probe() {
  ...
  if [ "$second_payload" != "-" ]; then
    set -- --second "$second_payload"
  else
    set --
  fi
...
build_probe \
  phase3-atag-only-zimage \
  "$zimage" \
  -

build_probe \
  phase3-dtb-in-second \
  "$zimage" \
  "$dtb"

build_probe \
  phase3-appended-dtb-second-duplicate \
  "$zimage_dtb" \
  "$dtb"
...
printf 'Phase 3 ATAG/DTB placement recovery candidates are ready:\n'
