...
kernel_out=${KERNEL_OUT:-build/phase3/enable-mmu-entry-reset-kernel-out}
...
out_dir='build/phase3/enable-mmu-entry-reset'
...
patch_note="$out_dir/enable-mmu-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-enable-mmu-entry-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM enable-mmu-entry marker")
if ".Lis01_enable_mmu_entry_reset:" in text:
    raise SystemExit("error: IS01 enable-mmu-entry reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if decompressed kernel head.S branches
		 * into __enable_mmu, trigger the downstream proc_comm reset
		 * path before loading domain/page-table registers.
		 */
.Lis01_enable_mmu_entry_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\tb\t.Lis01_reset_loop
"""
replacement = needle.replace("__enable_mmu:", "__enable_mmu:" + reset)
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S at __enable_mmu entry to trigger "
    "proc_comm reset before domain/page-table register setup.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-enable-mmu-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-enable-mmu-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-enable-mmu-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-enable-mmu-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-enable-mmu-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-enable-mmu-entry-reset-ubinize.cfg"
...
printf 'Phase 3 enable-mmu-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
