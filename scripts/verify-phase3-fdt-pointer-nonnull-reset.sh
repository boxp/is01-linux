...
grep -F 'dt_virt is non-NULL' "$patch_note" >/dev/null || fail 'patch note does not describe dt_virt non-NULL checkpoint'
grep -F 'early_init_dt_verify()' "$patch_note" >/dev/null || fail 'patch note does not describe early_init_dt_verify checkpoint'
grep -F 'valid FDT' "$patch_note" >/dev/null || fail 'patch note does not describe valid FDT validation'
...
printf 'Phase 3 fdt-pointer-nonnull reset candidate verified:\n'
