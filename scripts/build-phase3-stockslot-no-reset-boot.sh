...
if len(data) > slot_size:
    raise SystemExit(f"zImage is larger than stock kernel slot: {len(data)} > {slot_size}")
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-no-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-no-reset-boot-volume.bin"
ubi_img="$out_dir/phase3-stockslot-no-reset-boot-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-no-reset-boot-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-no-reset-boot-ubinize.cfg"
...
printf 'Phase 3 stock-slot no-reset boot recovery candidate is ready:\n'
cat "$source_note"
