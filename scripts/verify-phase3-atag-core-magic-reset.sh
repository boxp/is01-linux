...
grep -F 'dt_virt pointer' "$patch_note" >/dev/null || fail 'patch note does not describe dt_virt checkpoint'
grep -F 'ATAG_CORE' "$patch_note" >/dev/null || fail 'patch note does not describe ATAG_CORE checkpoint'
grep -F 'early_init_dt_verify()' "$patch_note" >/dev/null || fail 'patch note does not describe early_init_dt_verify checkpoint'
grep -F 'FDT' "$patch_note" >/dev/null || fail 'patch note does not describe FDT validation'
...
printf 'Phase 3 atag-core-magic reset candidate verified:\n'
