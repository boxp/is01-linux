...
grep -F 'invalid-machine handling' "$patch_note" >/dev/null || fail 'patch note does not describe invalid-machine checkpoint'
grep -F 'setup_machine_tags() returns NULL' "$patch_note" >/dev/null || fail 'patch note does not describe NULL mdesc checkpoint'
...
printf 'Phase 3 mdesc-null reset candidate verified:\n'
