...
test -s "$zimage_dtb" || ./scripts/build-phase3-mainline-lean-boot.sh
test -s "$dtb" || ./scripts/build-phase3-mainline-lean-boot.sh
...
pad_slot() {
  ...
  python3 - "$src" "$dst" "$slot_size" "$label" <<'PY'
from pathlib import Path
import sys
...
if len(data) > slot_size:
    raise SystemExit(f"{label} payload is larger than stock slot: {len(data)} > {slot_size}")
dst.write_bytes(data + (b"\0" * (slot_size - len(data))))
PY
}

pad_slot "$zimage" "$padded_zimage" "$stock_kernel_size" zImage
pad_slot "$zimage_dtb" "$padded_zimage_dtb" "$stock_kernel_size" zImage-dtb
pad_slot "$ramdisk" "$padded_ramdisk" "$stock_ramdisk_size" ramdisk
...
build_candidate \
  phase3-stockslot-zimage-only-phase3-cmdline \
  "$padded_zimage"

build_candidate \
  phase3-stockslot-zimage-second-dtb-phase3-cmdline \
  "$padded_zimage" \
  "$dtb"

build_candidate \
  phase3-stockslot-zimage-appended-second-dtb-phase3-cmdline \
  "$padded_zimage_dtb" \
  "$dtb"
...
printf 'Phase 3 stock-header DTB handoff recovery candidates are ready:\n'
cat "$source_note"
