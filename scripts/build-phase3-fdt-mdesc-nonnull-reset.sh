...
kernel_out=${KERNEL_OUT:-build/phase3/fdt-mdesc-nonnull-reset-kernel-out}
...
out_dir='build/phase3/fdt-mdesc-nonnull-reset'
...
patch_note="$out_dir/fdt-mdesc-nonnull-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-fdt-mdesc-nonnull-reset-stock-kernel-slot.bin"
...
python3 - "$head_s" "$setup_c" "$patch_note" <<'PY'
...
if ".Lis01_map_proc_comm_for_fdt_mdesc_nonnull_reset:" in text:
    raise SystemExit("error: IS01 fdt-mdesc-nonnull reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_fdt_mdesc_nonnull_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 fdt-mdesc-nonnull proc_comm reset probe needs non-LPAE section mappings
#endif
...
note.write_text(
    ...
    "and MSM_CSR in early page tables, then patched arch/arm/kernel/setup.c "
    "to trigger proc_comm reset only when setup_machine_fdt() returns a "
...
boot_img="$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-fdt-mdesc-nonnull-reset-ubinize.cfg"
...
printf 'Phase 3 fdt-mdesc-nonnull reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
