...
out_dir='build/phase2/buildroot-recovery'
rootfs_out=${PHASE2_BUILDROOT_OUT:-build/phase2/buildroot}
boot_img="$out_dir/phase2-buildroot-stock-kernel-boot.img"
ubi_payload="$out_dir/phase2-buildroot-stock-kernel-volume.bin"
raw_ramdisk="$out_dir/phase2-buildroot-rootfs.img"
recovery_img="$out_dir/phase2-buildroot-stock-kernel-recovery.img"
extracted_ramdisk="$out_dir/extracted-phase2-buildroot-rootfs.img"
roundtrip_boot="$out_dir/roundtrip-phase2-buildroot-stock-kernel-boot.img"
expected_cpio="$rootfs_out/images/rootfs.cpio"
...
file "$boot_img" | grep 'Android bootimg' >/dev/null || fail 'Phase 2 Buildroot boot image is not an Android bootimg'
file "$recovery_img" | grep 'UBI image' >/dev/null || fail 'Phase 2 Buildroot recovery image is not a UBI image'
...
cmp "$boot_img" "$roundtrip_boot" || fail 'repacked UBI volume does not round-trip to Phase 2 Buildroot boot image'
...
cmp "$raw_ramdisk" "$extracted_ramdisk" || fail 'boot image ramdisk does not match raw Phase 2 Buildroot rootfs'
cmp "$expected_cpio" "$extracted_ramdisk" || fail 'boot image ramdisk payload does not match Buildroot cpio'
...
printf 'Phase 2 Buildroot recovery artifacts verified:\n'
