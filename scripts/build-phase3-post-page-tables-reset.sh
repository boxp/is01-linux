...
kernel_out=${KERNEL_OUT:-build/phase3/post-page-tables-reset-kernel-out}
...
out_dir='build/phase3/post-page-tables-reset'
...
patch_note="$out_dir/post-page-tables-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-page-tables-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """	bl	__create_page_tables
"""
if needle not in text:
    raise SystemExit("error: could not find ARM post-create-page-tables marker")
if ".Lis01_post_page_tables_reset:" in text:
    raise SystemExit("error: IS01 post-page-tables reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if decompressed kernel head.S returns
		 * from __create_page_tables, trigger the downstream proc_comm
		 * reset path before CPU-specific init and MMU enable.
		 */
.Lis01_post_page_tables_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
note.write_text(
    "Patched arch/arm/kernel/head.S after __create_page_tables returns "
    "to trigger proc_comm reset before CPU-specific init and MMU enable.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-page-tables-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-page-tables-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-page-tables-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-page-tables-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-page-tables-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-page-tables-reset-ubinize.cfg"
...
printf 'Phase 3 post-page-tables reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
