...
kernel_out=${KERNEL_OUT:-build/phase3/machine-type-deckard-reset-kernel-out}
...
out_dir='build/phase3/machine-type-deckard-reset'
...
patch_note="$out_dir/machine-type-deckard-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-machine-type-deckard-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_machine_type_deckard_reset:" in text:
    raise SystemExit("error: IS01 machine-type-deckard reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_machine_type_deckard_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 machine-type-deckard proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: setup_machine_fdt() returned NULL and the
\t * fallback setup_machine_tags() path is about to run. Trigger the
\t * downstream proc_comm reset only when the bootloader-provided
\t * machine type equals MACH_DECKARD (2008030).
\t */
\tasm volatile(
\t\"mrs r0, cpsr\\n\"
\t\"orr r0, r0, #0xc0\\n\"
...
if "IS01 Phase 3 probe: setup_machine_fdt() returned NULL and the" in setup_text:
    raise SystemExit("error: IS01 machine-type-deckard reset patch is already present")
setup_pattern = re.compile(
    r"(\n\tif \(!mdesc\))\n"
    r"(\t\tmdesc = setup_machine_tags\(atags_vaddr, __machine_arch_type\);\n)",
    re.MULTILINE,
)
setup_text, count = setup_pattern.subn(
    lambda match: (
        match.group(1)
        + "\t{\n"
        + "\t\tif (__machine_arch_type == 2008030) {\n"
        + reset
        + "\t\t}\n"
        + match.group(2)
        + "\t}\n"
...
if count != 1:
    raise SystemExit("error: could not find setup_machine_tags fallback in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset before setup_machine_tags() only when "
    "__machine_arch_type equals MACH_DECKARD (2008030), after "
    "setup_machine_fdt() returned NULL.\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-machine-type-deckard-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-machine-type-deckard-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-machine-type-deckard-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-machine-type-deckard-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-machine-type-deckard-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-machine-type-deckard-reset-ubinize.cfg"
...
printf 'Phase 3 machine-type-deckard reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
