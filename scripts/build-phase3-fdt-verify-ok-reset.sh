...
kernel_out=${KERNEL_OUT:-build/phase3/fdt-verify-ok-reset-kernel-out}
...
out_dir='build/phase3/fdt-verify-ok-reset'
...
patch_note="$out_dir/fdt-verify-ok-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-fdt-verify-ok-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_fdt_verify_ok_reset:" in text:
    raise SystemExit("error: IS01 fdt-verify-ok reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_fdt_verify_ok_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 fdt-verify-ok proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: early_init_dt_verify() accepted the
\t * atags_vaddr pointer as an FDT. Trigger the downstream proc_comm
\t * reset before of_flat_dt_match_machine() so the next split can
\t * distinguish invalid-DTB from machine-match failure.
\t */
\tasm volatile(
\t\"mrs r0, cpsr\\n\"
\t\"orr r0, r0, #0xc0\\n\"
...
if "IS01 Phase 3 probe: early_init_dt_verify() accepted" in devtree_text:
    raise SystemExit("error: IS01 fdt-verify-ok reset patch is already present")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately after early_init_dt_verify() "
    "accepts the setup_machine_fdt() pointer as a valid FDT, before "
...
boot_img="$out_dir/phase3-stockslot-fdt-verify-ok-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-fdt-verify-ok-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-fdt-verify-ok-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-fdt-verify-ok-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-fdt-verify-ok-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-fdt-verify-ok-reset-ubinize.cfg"
...
printf 'Phase 3 fdt-verify-ok reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
