...
extracted_ramdisk="$out_dir/extracted-phase1-stock-kernel-raw-initramfs.img"
roundtrip_boot="$out_dir/roundtrip-phase1-stock-kernel-raw-initramfs-boot.img"
expected_cpio='build/phase1/initramfs/initramfs.cpio'
...
file "$boot_img" | grep 'Android bootimg' >/dev/null || fail 'stock-kernel raw-initramfs boot image is not an Android bootimg'
file "$recovery_img" | grep 'UBI image' >/dev/null || fail 'stock-kernel raw-initramfs recovery image is not a UBI image'
...
cmp "$boot_img" "$roundtrip_boot" || fail 'repacked UBI volume does not round-trip to stock-kernel raw-initramfs boot image'
...
PY
cmp "$raw_ramdisk" "$extracted_ramdisk" || fail 'boot image ramdisk does not match raw Phase 1 initramfs'
cmp "$expected_cpio" "$extracted_ramdisk" || fail 'boot image ramdisk payload does not match Phase 1 cpio'

sha256sum "$boot_img" "$ubi_payload" "$recovery_img" "$roundtrip_boot" "$extracted_ramdisk" >"$out_dir/SHA256SUMS"

printf 'Phase 1 stock-kernel raw-initramfs artifacts verified:\n'
