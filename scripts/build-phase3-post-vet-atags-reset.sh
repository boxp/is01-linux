...
kernel_out=${KERNEL_OUT:-build/phase3/post-vet-atags-reset-kernel-out}
...
out_dir='build/phase3/post-vet-atags-reset'
...
patch_note="$out_dir/post-vet-atags-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-vet-atags-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM post-vet-atags marker")
if ".Lis01_post_vet_atags_reset:" in text:
    raise SystemExit("error: IS01 post-vet-atags reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if decompressed kernel head.S returns
		 * from __vet_atags, trigger the downstream proc_comm reset
		 * path before SMP/PV fixups and early page table setup.
		 */
.Lis01_post_vet_atags_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
note.write_text(
    "Patched arch/arm/kernel/head.S after __vet_atags returns "
    "to trigger proc_comm reset before SMP/PV fixups and early page table setup.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-vet-atags-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-vet-atags-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-vet-atags-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-vet-atags-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-vet-atags-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-vet-atags-reset-ubinize.cfg"
...
printf 'Phase 3 post-vet-atags reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
