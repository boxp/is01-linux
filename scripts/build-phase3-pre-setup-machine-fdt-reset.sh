...
kernel_out=${KERNEL_OUT:-build/phase3/pre-setup-machine-fdt-reset-kernel-out}
...
out_dir='build/phase3/pre-setup-machine-fdt-reset'
...
patch_note="$out_dir/pre-setup-machine-fdt-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-pre-setup-machine-fdt-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_pre_setup_machine_fdt_reset:" in text:
    raise SystemExit("error: IS01 pre-setup-machine-fdt reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_pre_setup_machine_fdt_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 pre-setup-machine-fdt proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: if execution reaches the FDT machine
\t * descriptor path, trigger the downstream proc_comm reset path
\t * immediately before setup_machine_fdt().
...
if "IS01 Phase 3 probe: if execution reaches the FDT machine" in setup_text:
    raise SystemExit("error: IS01 pre-setup-machine-fdt reset patch is already present")
setup_pattern = re.compile(r"(\n\tif \(__atags_pointer\)\n\t\tatags_vaddr = FDT_VIRT_BASE\(__atags_pointer\);\n\n\tsetup_processor\(\);\n\tif \(atags_vaddr\) \{\n)", re.MULTILINE)
setup_text, count = setup_pattern.subn(
    lambda match: match.group(1) + reset,
    setup_text,
    count=1,
)
if count != 1:
    raise SystemExit("error: could not find setup_machine_fdt pre-call block in ARM setup_arch")
...
note.write_text(
    ...
    "to trigger proc_comm reset immediately before setup_machine_fdt().\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-pre-setup-machine-fdt-reset-ubinize.cfg"
...
printf 'Phase 3 pre-setup-machine-fdt reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
