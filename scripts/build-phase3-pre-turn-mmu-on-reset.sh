...
kernel_out=${KERNEL_OUT:-build/phase3/pre-turn-mmu-on-reset-kernel-out}
...
out_dir='build/phase3/pre-turn-mmu-on-reset'
...
patch_note="$out_dir/pre-turn-mmu-on-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-pre-turn-mmu-on-reset-stock-kernel-slot.bin"
...
if needle not in text:
    raise SystemExit("error: could not find ARM pre-turn-mmu-on marker")
if ".Lis01_pre_turn_mmu_on_reset:" in text:
    raise SystemExit("error: IS01 pre-turn-mmu-on reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if __enable_mmu finishes loading
		 * domain/page-table registers, trigger the downstream
		 * proc_comm reset path before branching to __turn_mmu_on.
		 */
.Lis01_pre_turn_mmu_on_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\tb\t.Lis01_reset_loop
"""
replacement = needle.replace("\tb\t__turn_mmu_on", reset + "\n\tb\t__turn_mmu_on")
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S before the __turn_mmu_on branch to "
    "trigger proc_comm reset after domain/page-table register setup.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-pre-turn-mmu-on-reset-ubinize.cfg"
...
printf 'Phase 3 pre-turn-mmu-on reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
