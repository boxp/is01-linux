...
kernel_out=${KERNEL_OUT:-build/phase3/pre-setup-arch-reset-kernel-out}
...
out_dir='build/phase3/pre-setup-arch-reset'
...
patch_note="$out_dir/pre-setup-arch-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-pre-setup-arch-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_pre_setup_arch_reset:" in text:
    raise SystemExit("error: IS01 pre-setup-arch reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_pre_setup_arch_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 pre-setup-arch proc_comm reset probe needs non-LPAE section mappings
#endif
...
\t * before setup_arch(), trigger the downstream proc_comm reset path.
...
note.write_text(
    ...
    "trigger proc_comm reset immediately before setup_arch().\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-pre-setup-arch-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-pre-setup-arch-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-pre-setup-arch-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-pre-setup-arch-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-pre-setup-arch-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-pre-setup-arch-reset-ubinize.cfg"
...
printf 'Phase 3 pre-setup-arch reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
