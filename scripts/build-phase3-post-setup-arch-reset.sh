...
kernel_out=${KERNEL_OUT:-build/phase3/post-setup-arch-reset-kernel-out}
...
out_dir='build/phase3/post-setup-arch-reset'
head_s="$kernel_src/arch/arm/kernel/head.S"
main_c="$kernel_src/init/main.c"
...
test -s "$main_c" || {
  printf 'error: missing Linux init/main.c: %s\n' "$main_c" >&2
  exit 1
}

mkdir -p "$kernel_out" "$out_dir"
patch_note="$out_dir/post-setup-arch-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-post-setup-arch-reset-stock-kernel-slot.bin"
...
restore_head() {
  ...
  if [ -n "${main_c_backup:-}" ] && [ -f "$main_c_backup" ]; then
...
main_c_backup=$(mktemp)
cp "$main_c" "$main_c_backup"

python3 - "$head_s" "$main_c" "$patch_note" <<'PY'
...
if ".Lis01_map_proc_comm_for_post_setup_arch_reset:" in text:
    raise SystemExit("error: IS01 post-setup-arch reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_post_setup_arch_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 post-setup-arch proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: if execution reaches the point immediately
\t * after setup_arch(), trigger the downstream proc_comm reset path.
...
main_needle = """	setup_arch(&command_line);
"""
if main_needle not in main_text:
    raise SystemExit("error: could not find setup_arch call in start_kernel")
if "IS01 Phase 3 probe: if execution reaches the point immediately" in main_text:
    raise SystemExit("error: IS01 post-setup-arch reset patch is already present")
main_c.write_text(
    main_text.replace(
        main_needle,
...
note.write_text(
    ...
    "and MSM_CSR in early page tables, then patched init/main.c to "
    "trigger proc_comm reset immediately after setup_arch().\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-post-setup-arch-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-post-setup-arch-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-post-setup-arch-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-post-setup-arch-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-post-setup-arch-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-post-setup-arch-reset-ubinize.cfg"
...
printf 'Phase 3 post-setup-arch reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
