...
grep -F 'setup_machine_tags() returns' "$patch_note" >/dev/null || fail 'patch note does not describe setup_machine_tags return checkpoint'
grep -F 'before invalid-machine handling' "$patch_note" >/dev/null || fail 'patch note does not describe invalid-machine checkpoint'
...
printf 'Phase 3 post-setup-machine-tags reset candidate verified:\n'
