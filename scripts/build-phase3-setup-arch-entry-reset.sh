...
kernel_out=${KERNEL_OUT:-build/phase3/setup-arch-entry-reset-kernel-out}
...
out_dir='build/phase3/setup-arch-entry-reset'
...
patch_note="$out_dir/setup-arch-entry-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-setup-arch-entry-reset-stock-kernel-slot.bin"
...
if ".Lis01_map_proc_comm_for_setup_arch_entry_reset:" in text:
    raise SystemExit("error: IS01 setup-arch-entry reset patch is already present")
...
maps = """
	 ...
	 */
.Lis01_map_proc_comm_for_setup_arch_entry_reset:
#ifdef CONFIG_ARM_LPAE
#error IS01 setup-arch-entry proc_comm reset probe needs non-LPAE section mappings
#endif
...
reset = """
\t/*
\t * IS01 Phase 3 probe: if execution reaches the entry of
...
if "IS01 Phase 3 probe: if execution reaches the entry of" in setup_text:
    raise SystemExit("error: IS01 setup-arch-entry reset patch is already present")
setup_pattern = re.compile(
    r"(void __init setup_arch\(char \*\*cmdline_p\)\n\{\n)",
    re.MULTILINE,
)
...
if count != 1:
    raise SystemExit("error: could not find setup_arch function entry in ARM setup.c")
...
note.write_text(
    ...
    "to trigger proc_comm reset at setup_arch() entry.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-setup-arch-entry-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-setup-arch-entry-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-setup-arch-entry-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-setup-arch-entry-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-setup-arch-entry-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-setup-arch-entry-reset-ubinize.cfg"
...
printf 'Phase 3 setup-arch-entry reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
