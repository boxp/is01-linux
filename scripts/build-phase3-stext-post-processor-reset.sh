...
kernel_out=${KERNEL_OUT:-build/phase3/stext-post-processor-reset-kernel-out}
...
out_dir='build/phase3/stext-post-processor-reset'
...
patch_note="$out_dir/stext-post-processor-reset-patch.txt"
...
padded_kernel="$out_dir/phase3-stext-post-processor-reset-stock-kernel-slot.bin"
...
text = head.read_text()
needle = """	mrc	p15, 0, r9, c0, c0		@ get processor id
...
if needle not in text:
    raise SystemExit("error: could not find ARM post-processor-lookup marker")
if ".Lis01_stext_post_processor_reset:" in text:
    raise SystemExit("error: IS01 stext-post-processor reset patch is already present")

reset = """
		/*
		 * IS01 Phase 3 probe: if decompressed kernel head.S reaches
		 * the point after __lookup_processor_type succeeds, trigger
		 * the downstream proc_comm reset path before ATAG/DT vetting
		 * and early page table setup.
		 */
.Lis01_stext_post_processor_reset:
\t\tmrs\tr0, cpsr
\t\torr\tr0, r0, #0xc0
\t\tmsr\tcpsr_c, r0
...
\t\tb\t.Lis01_reset_loop
"""
replacement = needle + reset + "\n"
head.write_text(text.replace(needle, replacement, 1))
note.write_text(
    "Patched arch/arm/kernel/head.S after __lookup_processor_type success "
    "to trigger proc_comm reset before ATAG/DT vetting and early page table setup.\\n"
)
PY
...
boot_img="$out_dir/phase3-stockslot-stext-post-processor-reset-boot.img"
...
./scripts/inspect-android-bootimg.py \
  ...
  "$boot_img" >"$out_dir/phase3-stockslot-stext-post-processor-reset-boot-layout.txt"

ubi_payload="$out_dir/phase3-stockslot-stext-post-processor-reset-volume.bin"
ubi_img="$out_dir/phase3-stockslot-stext-post-processor-reset-recovery.ubi"
recovery_img="$out_dir/phase3-stockslot-stext-post-processor-reset-recovery.img"
ubinize_cfg="$out_dir/phase3-stockslot-stext-post-processor-reset-ubinize.cfg"
...
printf 'Phase 3 stext post-processor reset recovery candidate is ready:\n'
cat "$source_note"
cat "$patch_note"
